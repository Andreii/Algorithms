...
public class _31_next_permutation {
    // TC: O(n)
    // SC: O(1)
    // failing test -> to correct
    public void nextPermutation(int[] nums) {
        if(nums.length == 1) return;

        int n = nums.length;
        int swapIndex = findPeak(nums);
        ...
        if(swapIndex == 0) {
            ...
            for(int i = swapIndex + 1; i < n; i++) {
                if(nums[i] < nums[swapIndex] && nums[i] > nums[prev]) {
                    swapIndex = i;
                }
            }

            swap(prev, swapIndex, nums);
    ...
    public void swap(int p, int s, int[] nums) {
        int z = nums[p];
        nums[p] = nums[s];
        nums[s] = z;
    }
